...
export default function Dashboard(){
  const {user}=useAuth(); const [data,setData]=useState({files:[],folders:[]}); const [stars,setStars]=useState([]); const [loading,setLoading]=useState(true);
  useEffect(()=>{Promise.all([getFiles(),getStars()]).then(([f,s])=>{setData(x=>({...x,files:f.data.files||f.data||[]}));setStars(s.data.stars||s.data||[])}).finally(()=>setLoading(false))},[]);
  ...
  return <div className="fade-in space-y-8">
    ...
    <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
      <Stat icon={Files} label="Files" value={data.files.length}/>
      <Stat icon={Folder} label="Folders" value="—"/>
      <Stat icon={Star} label="Starred" value={stars.length}/>
      <Stat icon={HardDrive} label="Storage" value={formatBytes(data.files.reduce((a,x)=>a+(x.size||0),0))}/>
    </div>
    <div className="grid xl:grid-cols-[1fr_330px] gap-5">
      <section className="bg-white border border-[#dedfd7] rounded-[24px] overflow-hidden">
        ...
        {loading?<div className="p-5 space-y-3">{[1,2,3,4].map(i=><div className="h-12 rounded-xl skeleton" key={i}/>)}</div>:recent.length?<div>{recent.map(f=><Link to={`/files?open=${f.id}`} key={f.id} className="flex items-center gap-4 p-4 hover:bg-black/[.02] border-b border-[#eee] last:border-0"><div className="w-10 h-10 rounded-xl bg-[#f0f1e9] grid place-items-center"><Files size={18}/></div><div className="flex-1 min-w-0"><div className="text-sm font-medium truncate">{f.name}</div><div className="text-xs text-black/40">{formatBytes(f.size)} · {formatDate(f.updated_at || f.created_at)}</div></div><ArrowUpRight size={16} className="text-black/25"/></Link>)}</div>:<div className="p-10 text-center text-sm text-black/40">No files yet.</div>}
      </section>
